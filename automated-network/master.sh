...
for line in /NTI-320-linux-monitoring/automated-network/configs/instances-list; do
  HOSTNAME=$line
...
#These check to see if $HOSTNAME matches the case. If so, it launches the instance with required conditions.
  ...
  if [ $HOSTNAME != 'ldap' ] && [ $HOSTNAME != 'nfs' ] && [ $HOSTNAME != 'postgres' ] && [ $HOSTNAME != 'django' ] && [ $HOSTNAME != 'load-balancer' ] && [ $HOSTNAME != 'client' ]; then
